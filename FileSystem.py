...
class Block:
    """
    物理块的类，每个物理块包含一个编号和一个数据部分
    为了方便实现，将数据部分全部定义为字符串类型
    """
    ...
    def append(self, newData: str) -> str:
        ...
        # 如果当前物理块可用空间大于新内容的长度，则直接将新内容追加到当前物理块中
        if remainSpace >= len(newData):
            self.data += newData
            return ""
        else:
            # 如果当前物理块可用空间小于新内容的长度，则将新内容追加到当前物理块中，并返回剩余的新内容
...
class FAT:
    ...
    def write(self, data, disk):
        ...
        while data != "":
            ...
            if cur != -1:
                ...
                start = newLoc
            cur = newLoc
            data = disk[cur].write(data)  # 向物理盘块中写入数据，返回未写入的数据，继续分配盘块
            self.fat[cur] = -1  # 当前块写入数据，标记为已经占用

        return start  # 返回文件占用的第一个磁盘块
    ...
    def delete(self, start, disk):
        ...
        while self.fat[start] != -1:
            disk[start].clear()  # 清空物理块中的数据
            las = self.fat[start]
            self.fat[start] = -2  # 标记为空闲盘块
            start = las  # 得到下一个磁盘块的位置
...
class FCB:
    ...
    文件控制块
    """
    def __init__(self, name, createTime, data, fat, disk, authority=2):
        # 文件名
        self.name = name
        # 创建时间
        ...
        # 文件权限
        self.authority = authority  # 0表示可读，1表示可写，2表示可读可写，默认文件为可读可写
...
class MFDBlock:
    """
    主文件目录，这里用来存储用户的信息，每个用户对应一个Catalog目录结点，这里表示一个主文件目录块
    """
    def __init__(self, user='root', passord=''):
        self.user = user
        self.passord = passord
        # 每个用户对应一个根目录结点，然后每个用户可以在记得的根目录下创建自己的文件系统
        self.catalog = Catalog(user, False, None, None, time.time())

MFD = [MFDBlock('root')]

# 接下来制作文件系统可视化ui，这里模拟的是windows系统的文件资源管理器ui
...
class EditingInterface(QWidget):
    ...
    def __init__(self, name, data):
        ...
        # 这里是设置为模态窗口，当前窗口打开的时候，其他窗口无法进行交互，直到关闭
...
class AttributeInterface(QWidget):
    '''这里设计的是显示文件详细信息的ui
    这里需要传入文件名，是否是文件，创建时间，修改时间，子文件夹数量'''
    ...
    # 时间类型数据的转换函数，转换为时间字符串，用来显示在属性界面
...
class FileSystem(QMainWindow):
    def __init__(self, current_user='root'):
        ...
        self.baseUrl = [self.current_user]  # 根目录
    ...
    def init_ui(self):
        ...
        def set_menu():
            ...
            menubar.addAction('登录', self.login)  # 模拟用户登录
        ...
        def set_navigation_bar():
            # 设置一个地址树部件，用来显示多级文件目录结构
            self.tree = QTreeWidget()
            # 设置标题
            self.tree.setHeaderLabels(['当前用户：' + self.current_user])
            # 设置列数
        ...
        def set_file_info():
            '''文件列表信息的设置，展示当前目录下的所有目录和文件信息'''
            self.listView = ListWidget(self.curNode, parents=self)  # 创建一个ListWidget对象
            ...
            # 加载右击菜单
            self.listView.setContextMenuPolicy(Qt.CustomContextMenu)  # 设置为自定义菜单
            self.listView.customContextMenuRequested.connect(self.show_menu)  # 鼠标右键关联文件菜单
    ...
    def project_init(self):
        ...
        if not os.path.exists('fat'):
            self.fat = FAT()
            self.fat.fat = [-2] * blockNum  # 初始化一个大小为blockNum大小的FAT表
            # 存储fat表
            with open('fat', 'wb') as f:
                f.write(pickle.dumps(self.fat))  # 将对象序列化为字节流，保存本次FAT表的状况，下面代码逻辑相同
        else:
        ...
        # 打开对应用户的UFD，然后读入对应的结点信息
        if not os.path.exists(self.current_user):  # 表示没有该用户
            self.catalog = []
            self.catalog.append(Catalog(self.current_user, False, self.fat, self.disk, time.localtime(time.time())))
            # 存储
            with open(self.current_user, 'wb') as f:
                f.write(pickle.dumps(self.catalog))
        else:
            with open(self.current_user, 'rb') as f:
        ...
        def buildTreeRecursive(node: Catalog, parent: QTreeWidgetItem):
            ...
            if node.isFile:
                ...
                for i in node.children:
                    # 递归调用，建立文件树，我的设计是将子目录存储在列表中，也可以有链表存储
    ...
    def login(self):
        ...
        login.exec_()
        login.show()
        login_user = login.input_line_edit.text()
        self.current_user = login_user
        # 判断该用户是否存在，如果存在，切换为该用户的目录环境，如果不存在，新建一个用户UFD，同时将新用户添加到MFD中
        for user in MFD:
            if user.user == login_user:
                # 重新加载页面
                self.hide()
                self.main_window = FileSystem(current_user=login_user)
                self.main_window.show()
        ...
        MFD.append(MFDBlock(login_user))
        # 重新加载页面
        self.hide()
        self.main_window = FileSystem(self.current_user)
        self.main_window.show()
    ...
    def click_item(self, item, column):
        ...
        for i in ways:
            ...
            if newNode is not None and not newNode.isFile:
                ...
                self.baseUrl.append(newNode.name)  # 表示绝对路径
        ...
        self.update_address_bar()
        # 设置回退/前进键的可用状态
        self.back_action.setEnabled(self.curNode != self.rootNode)  # 更换目录之后，就可能需要回退
        self.forward_action.setEnabled(False)
        # 将上一次的位置记为lastLoc
        self.lastLoc = -1
    ...
    def open_file(self, modelindex: QModelIndex) -> None:
        ...
        if newNode.isFile:
            ...
            self.child = EditingInterface(newNode.name, data)  # 弹出文件编辑对话框
            self.child._signal.connect(getData)  # 连接信号槽，当用户编辑完文件之后，得到文件内的数据，写入盘块
            self.child.show()  # 显示内容编辑框
    ...
    def delete(self):
        ...
        # 如果是框选，就删除最后一个框选的文件后者文件夹，这里还可以改进
        item = self.listView.selectedItems()[-1]
